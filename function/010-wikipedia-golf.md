...
const files = dv.pages()
    ...
    .filter(p => !p.file.folder.match(/function/))
...
// make a graph
files.forEach(f => {
	let outlinks = f.outlinks.filter(link => dv.page(link)).filter(link => !link.path.includes("-moc-") && !link.path.includes("020-symptom-pairs"));
	let inlinks = f.inlinks.filter(link => dv.page(link)).filter(link => !link.path.includes("-moc-") && !link.path.includes("020-symptom-pairs"));
